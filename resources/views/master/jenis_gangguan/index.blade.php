            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Jenia Gangguan</h3>
                </div>
                <div class="card-body">
                        ...
                        <table id="example" class="table table-vcenter card-table table-striped">
                          <thead>
                            <tr>
                              <th class="w-1"></th>
                              <th>
                                <input type="text" class="form-control filter-input" placeholder="Jenis Gangguan" data-column="1">
                              </th>
                              <th>
                                <input type="text" class="form-control filter-input" placeholder="Keterangan" data-column="2">
                              </th>
                              @can('master_jenis_gangguan-edit')
                                  <th ></th>
                              @endcan
                              @can('master_jenis_gangguan-delete')
                                  <th></th>
                              @endcan
                            </tr>
                            <tr>
...
@push('addon-script')
    ...
    <script>
        var datatables = $('#example').DataTable({
            ...
            buttons : [ 
                @can('master_jenis_gangguan-create')
                    {
                        text: 'Tambah',
                        className: 'btn btn-primary btn-sm btn-fw mb-4 mr-2 p-1',
                        action: function ( e, dt, button, config ) {
                            window.location = '{{ route('jenisGangguan.create') }}';
                        }        
                    },
                @endcan
                {
