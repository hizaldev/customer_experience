                        ...
                        <table id="example" class="table table-vcenter card-table table-striped">
                          <thead>
                            <tr>
                              <th class="w-1"></th>
                              <th>
                                <input type="text" class="form-control filter-input" placeholder="Kode Status" data-column="1">
                              </th>
                              <th>
                                <input type="text" class="form-control filter-input" placeholder="Statu" data-column="2">
                              </th>
                              <th>
                                <input type="text" class="form-control filter-input" placeholder="Keterangan" data-column="3">
                              </th>
                              @can('master_status-edit')
                                  <th ></th>
                              @endcan
                              @can('master_status-delete')
                                  <th></th>
                              @endcan
                            </tr>
                            <tr>
...
@push('addon-script')
    ...
    <script>
        var datatables = $('#example').DataTable({
            ...
            buttons : [ 
                @can('master_status-create')
                    {
                        text: 'Tambah',
                        className: 'btn btn-primary btn-sm btn-fw mb-4 mr-2 p-1',
                        action: function ( e, dt, button, config ) {
                            window.location = '{{ route('status.create') }}';
                        }        
                    },
                @endcan
                {
